   ...
   //solo puede ser necesario un String con absolutepath
	...
	static void closeFile( )
	{
		try{
			...
			LogData.epLog.append( "Cant close File: " + ioe );
	...
	static void writeVO( String tabla, ArrayList<JsField> fields ) throws IOException
	{
			...
			*/
			{
				...
				pw.println("	this.save = function( _original_callback )");
				pw.println("	{");
			...
			*/
			{
				...
				pw.println("				//console.log('ya termine el query de insert():',tx,results);");
				pw.println("				" + toCamelCase(tabla) + "._callback_stack.pop().call(null, "+tabla+");  ");
				pw.println("			});");
		...
		pw.println("	" + toCamelCase(tabla) + "._callback_stack = [];");
		...
		*/
		{
			...
			pw.println("	{");
			pw.println("		//console.log('estoy en getbypk()');");
			pw.println("		$sql = \"SELECT * FROM "+tabla+" WHERE ("+ sql + ") LIMIT 1;\";");
			pw.println("		db.query($sql, [" +pks+ "] , function(tx,results){ ");
			pw.println("				//console.log('ya termine el query de getByPK():',tx,results);");
			pw.println("				if(results.rows.length == 0) fres = null;");
			pw.println("				else fres = new "+toCamelCase(tabla)+"(results.rows.item(0)); ");
			pw.println("				" + toCamelCase(tabla) + "._callback_stack.length == 0 ? config.callback.call(config.context || null, fres)   "
										+  " : " + toCamelCase(tabla) + "._callback_stack.pop().call(config.context || null, fres);  ");
			pw.println("			});");
			pw.println("		return;");
			pw.println("	};");
			pw.println();
		...
		String fileName = "/DAO.js";
			...
			while( (line = sqlFile.readLine()) != null ){
				...
				if(line.trim().indexOf("enum") != -1) {
					LogData.epLog.append("\n\nEPA ! Enums no son soportados por Sqlite !*");
					continue;
				}
